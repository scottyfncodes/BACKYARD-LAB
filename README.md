...
| Workbench | Pick a part and hover where you want it: it auto-fits onto the nearest lock point in its most natural pose (wheels on axles, motors shaft-out). Click to place. `Q`/`E`/`Z`/`X`/`F` switch to fine tune (spin, tilt, mount side); arrows jump between lock points. Select a placed part and press `M` to move it. 💡 Idea walks you through a starter build | Tap a part in the tray, drag its ghost to where it should connect and let go: it clicks on in the most natural pose. 🎯 Fine tune gives a d-pad plus ↺ ↻ spin and ⤵ ⤴ tilt when you want control. Tap a placed part twice (or ✥ Move) to pick it back up with everything attached. 💡 Idea offers a step-by-step guided build (it opens automatically the first time) |
